...
namespace API.Controllers
{
    ...
    public class AlbumController : ControllerBase
    {
        ...
        {
            var albums = await _db.Albums.Select(x => new AlbumDto
            {
                Id = x.Id,
                Name = x.Name,
                PhotoUrl = x.PhotoUrl,
        ...
        public async Task<IActionResult> Create(AlbumAddEditDto model)
        {
            ...
            {
                return BadRequest("At least one artist id should be selected");
            }
...
if (fetchedAlbum == null)
            ...
            foreach(var artist  in fetchedAlbum.Artists) {
                var fetchedArtistAlbumBridge = await _db.ArtistAlbumBridge.SingleOrDefaultAsync(x => x.ArtistId == artist.ArtistId && x.AlbumId == fetchedAlbum.Id);
        ...
        {
            //removing any duplicate artistIds
